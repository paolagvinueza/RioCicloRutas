...
export default function detalhe() {
  ...
  const pegarCoordenadas = async () => {
    try {
      //const routes = await rota;
      //const midias = await midiass;
      //setRota(routes);
      //setMidias(midias);
  ...
  useEffect(() => {
    ...
    fetch('http://www.riobambaciclorutas.com/api/route/' + id + '.json')
      .then(response => response.json())
      .then(data => {
        const route = data.routes[0]; // Acessando o primeiro objeto da array
        setRota(route);
        console.log('Rotas -> ', data.routes);
      })
      .catch(error => console.log(error));

    fetch('http://www.riobambaciclorutas.com/api/coord/' + id + '.json')
      .then(response => response.json())
      .then(data => {
        //setCoordd(data.routescoordinates);
        console.log('Coordenadas dados -> ', data);
        console.log('Coordenadas -> ', 'http://www.riobambaciclorutas.com/api/coord/' + id + '.json');
      })
      .catch(error => console.log(error));

    fetch('http://www.riobambaciclorutas.com/api/midias/' + id + '.json')
      .then(response => response.json())
      .then(data => {
        setMidiass(data.midia);
        console.log('Mídias -> ', data.midia);
      })
      .catch(error => console.log(error));
